...
def bundle_css() -> str:
    ...
    paint. Shipping one pre-joined file removes that extra round trip.
    Files one directory below css/ (components/, sections/) reference
    assets as "../../..."; rewritten into the top-level bundle that becomes
    "../...", so it's adjusted by one level on the way in.
    """
    manifest = CSS_MANIFEST_PATH.read_text()
    parts = [
        "/* GENERATED by scripts/build.py from css/main.src.css.\n"
        " * Do not edit directly — edit main.src.css or the files it lists,\n"
