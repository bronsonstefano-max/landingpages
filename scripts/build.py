...
def fill_photo(template: str, photo: dict) -> str:
    return (
        template.replace("{{heroPhoto.src}}", photo["src"])
        .replace("{{heroPhoto.width}}", str(photo["width"]))
        .replace("{{heroPhoto.height}}", str(photo["height"]))
        .replace("{{heroPhoto.alt}}", html.escape(photo["alt"]))
    )
...
def render_services(names: list[str]) -> str:
    ...
    for name in names:
        rows.append(
            '              <a class="simple-list-item" href="tel:+18005555555" '
...
def render_steps(items: list[dict]) -> str:
    ...
    for i, item in enumerate(items, start=1):
        rows.append(
            ...
            f'                <span class="numbered-step-number" aria-hidden="true">{i}</span>\n'
            f"                <p><strong>{html.escape(item['title'], quote=False)}</strong> "
            f"{html.escape(item['body'], quote=False)}</p>\n"
            "              </li>"
        )
    return "\n".join(rows)
...
def _plain_phone(text: str) -> str:
    return text.replace("{{phone}}", "(800) 555-5555")
...
def render_faqs(items: list[dict]) -> str:
    ...
    for i, item in enumerate(items):
        ...
        answer = html.escape(item["answer"], quote=False).replace(
            "{{phone}}", PHONE_LINK.format(source=f"faq-{i + 1}")
        )
        rows.append(
...
def assemble() -> str:
    ...
    pieces = {
        ...
        "hero": fill_photo(load_partial("hero"), content["heroPhoto"]).replace(
